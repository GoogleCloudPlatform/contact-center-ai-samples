...
def get_project_number(token, project_id):
  ...
  if 'projectNumber' in r.json():
    ...
    return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'NO_PROJECT'}))}


def get_access_policy_name(token, access_policy_title, project_id):

  if not access_policy_title:
    return {"response": flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'NO_ACCESS_POLICY'}))}
  ...
  if not organization_id:
    return {"response": flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'NO_ORGANIZATION'}))}
  ...
  return {"response": flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'POLICY_NOT_FOUND'}))}
...
def get_service_perimeter_data_uri(token, project_id, access_policy_name):
  ...
  if r.status_code != 200:
    if (r.json()['error']['status'] == 'PERMISSION_DENIED') and (r.json()['error']['message'].startswith('Access Context Manager API has not been used in project')):
      response = flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'ACCESS_CONTEXT_MANAGER_API_DISABLED'}))
      return {'response':response}
    if r.json()['error']['status'] == 'PERMISSION_DENIED':
      response = flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'PERMISSION_DENIED'}))
      return {'response':response}
    else:
      logger.info(f'  accesscontextmanager API rejected request: {r.text}')
      response = flask.Response(status=500, response=r.text)
      return {'response':response}
  ...
  return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'PERIMETER_NOT_FOUND'}))}
...
def get_service_perimeter_status(token, project_id, access_policy_name):
  ...
  if r.status_code != 200:
    logger.info(f'  accesscontextmanager API rejected request: {r.text}')
    if (r.json()['error']['status'] == 'PERMISSION_DENIED') and (r.json()['error']['message'].startswith('Access Context Manager API has not been used in project')):
      return flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'ACCESS_CONTEXT_MANAGER_API_DISABLED'}))
    if r.json()['error']['status'] == 'PERMISSION_DENIED':
      response = flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'PERMISSION_DENIED'}))
      return {'response':response}
    else:
...
def check_function_exists(token, project_id, region, function_name):
  ...
  if r.status_code == 200:
    ...
    return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'WEBHOOK_NOT_FOUND'}))}
  elif r.status_code == 403 and r.json()['error']['message'].startswith('Cloud Functions API has not been used in project'):
    return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'CLOUDFUNCTIONS_API_DISABLED'}))}
    ...
    if (r.json()['error']['status'] == 'PERMISSION_DENIED') and (r.json()['error']['message'].startswith("Permission 'cloudfunctions.functions.get' denied on resource")):
      return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'PERMISSION_DENIED'}))}
    for details in r.json()['error']['details']:
      for violation in details['violations']:
        if violation['type'] == 'VPC_SERVICE_CONTROLS':
          return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'VPC_SERVICE_CONTROLS'}))}
    return {'response': flask.Response(status=500, response=r.text)}
  else:
...
def get_agents(token, project_id, region):
  ...
  if r.status_code == 403:
    if (r.json()['error']['status'] == 'PERMISSION_DENIED') and (r.json()['error']['message'].startswith('Dialogflow API has not been used in project')):
      response = flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'DIALOGFLOW_API_DISABLED'}))
      return {'response':response}
    if (r.json()['error']['status'] == 'PERMISSION_DENIED') and (r.json()['error']['message'].startswith('Caller does not have required permission')):
      response = flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'WRONG_PERMISSION'}))
      return {'response':response}
    for details in r.json()['error']['details']:
      for violation in details['violations']:
        if violation['type'] == 'VPC_SERVICE_CONTROLS':
  ...
  if len(result_dict) == 0:
    return {'response': flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'AGENT_NOT_FOUND'}))}
...
def get_webhooks(token, agent_name, project_id, region):
  ...
  if r.status_code == 403:
    for details in r.json()['error']['details']:
      for violation in details['violations']:
        if violation['type'] == 'VPC_SERVICE_CONTROLS':
          response = flask.Response(status=200, response=json.dumps({'status':'BLOCKED', 'reason':'VPC_SERVICE_CONTROLS'}))
          return {'response':response}
  if r.status_code != 200:
    logger.info(f'  dialogflow API rejected request: {r.text}')
    response = flask.Response(status=r.status_code, response=r.text)

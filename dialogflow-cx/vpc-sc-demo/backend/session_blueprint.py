...
def session_route():
    ...
    state = b64encode(
      json.dumps(
        {
          'return_to': login_landing_uri(flask.request, query_params=flask.request.args), 
          'session_id':session_id, 
          'public_pem':open(PUBLIC_PEM_FILENAME, 'r').read(),
        }
      ).encode()
    ).decode()
    response = flask.redirect(f'{AUTH_SERVICE_LOGIN_ENDPOINT}?state={state}')
    response.set_cookie("session_id", value=session_id, secure=True, httponly=True, domain=user_service_domain(flask.request))
    response.set_cookie("user_logged_in", value='true', secure=True, httponly=False, domain=user_service_domain(flask.request))
    return response
...
def logout():
  ...
  response = flask.redirect(login_landing_uri(flask.request, query_params=flask.request.args))

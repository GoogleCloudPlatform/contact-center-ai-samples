...
def update_webhook_access():  # pylint: disable=too-many-branches
    ...
    if response.status_code != 200:
        logger.info('  cloudfunctions API rejected getIamPolicy GET request: %s', response.text)
    ...
    if (
        (not internal_only and all_users_is_invoker_member) or
    ...
    if response.status_code != 200:
        logger.info(
            '  cloudfunctions API rejected setIamPolicy POST request: %s',
            response.text
        )
        return flask.Response(status=response.status_code, response=response.text)
    return flask.Response(status=200)
...
def update_service_directory_webhook_fulfillment():
    ...
    if fulfillment=='generic-web-service':
        ...
        data = {
            ...
            "serviceDirectory": {
                ...
                "genericWebService": {
                    ...
                    "allowedCaCerts": [
                        encode(
                            uu.get_cert(token, project_id, bucket)
                        )
                    ]
                }
            }
        ...
        return flask.Response(  # pragma: no cover
            status=500,
            response=f'Unexpected setting for fulfillment: {fulfillment}'
        )

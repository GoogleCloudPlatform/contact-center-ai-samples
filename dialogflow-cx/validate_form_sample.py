...
def build_fulfillment(text=None, webhook=None, tag=None):
    return Fulfillment(webhook=webhook, tag=tag, messages=[ResponseMessage(text=ResponseMessage.Text(text=text))])
...
class ValidateFormSample(sb.DialogflowSample):
    ...
    def __init__(self, project_id=None, quota_project_id=None, webhook_uri=None, agent_display_name=None):
      self.auth_delegator = sb.AuthDelegator(self, project_id=project_id, quota_project_id=quota_project_id, credentials=None)
      self.agent_delegator = sb.AgentDelegator(self, display_name=agent_display_name)
      self.webhook_delegator = sb.WebhookDelegator(self, display_name=self._WEBHOOK_DISPLAY_NAME, uri=webhook_uri)
      self.intent_delegator = sb.IntentDelegator(self, display_name=self._INTENT_DISPLAY_NAME)
      self.page_delegator = sb.FulfillmentPageDelegator(self, 
      ...
      for display_name, test_config in self.TEST_CASES.items():
        ...
        self.test_case_delegators[display_name] = sb.TestCaseDelegator(self,
    ...
    def initialize(self):
        ...
        self.page_delegator.add_parameter(
            ...
            fill_behavior=Form.Parameter.FillBehavior(initial_prompt_fulfillment=build_fulfillment(text=['What is your age?'])),
            default_value=None,
            redact=False,
            is_list=False,
        )
